...
- **Guided entry** — begin with search, a foundational core concept, a jurisdiction browser, or a comparative research question. The default view keeps the global index collapsed and reveals advanced corpus metadata, complete taxonomies, and dense visualizations only when requested; **Full workspace** restores the complete three-column research environment.
...
- **Law overview** — review an instrument's summary and legal status first, expand source and corpus details when needed, browse collapsible chapters with curated high-level summaries, and open an individual article or provision.
...
- **Workspace controls** — use task-oriented global navigation and contextual law/article controls, follow clickable breadcrumbs to any point in the current research path, switch between Guided and Full workspace density, and collapse or resize the left and right columns by pointer or keyboard.
...
The corpus reference date is **2026-07-28**. This means the dataset incorporates specifically documented developments through that date; it does not claim that every one of the 58 instruments received a fresh merits review on the same day. Each instrument, source audit and relation retains its own verification date.
...
- **255 curated source-linked anchors** plus version-locked complete corpora, producing **3,134 unique merged provision and framework nodes** after stable-ID de-duplication;
- **76 qualified relations**—47 candidate and 29 editorial-reviewed—plus **120 lifecycle events**;
- **58 instrument-level source-audit records**, 23 controlled core concepts in 7 themes, 34 curated structure summaries, and **3,134 clause-level topic classifications**: 2,363 substantive and 771 structural-context records; 44 records have an explicit editorial-review layer and 3,090 remain machine candidates. Candidate concept additions remain separately identifiable even on a reviewed record.

The repository now contains **39 version-locked complete corpora**. They cover the principal indexed binding laws and current/historical public-law texts across the EU, US, China, UK, Canada, Japan, India, Singapore, South Korea, Australia, Brazil, the Gulf states, Taiwan, Hong Kong SAR, Indonesia, Vietnam, South Africa, Nigeria and Switzerland. They also include the complete NIST AI RMF 1.0 hierarchy. Examples include GDPR (99 operative Articles), the EU AI Act (119 current operative Articles, 14 current Annexes and 180 enactment Recitals), China CSL (81), UK GDPR (120), PIPEDA (75 current units plus nine separately labelled enacted amendments not in force), the Canada ADM Directive (13 complete English/French units), Japan APPI (208), Australia Privacy Act (352), Singapore PDPA (106), Hong Kong PDPO (130 bilingual nodes), Swiss FADP (77 complete Articles plus two bounded Annex references), and the current Colorado AI Act session-law corpus.

The machine-generated English-corpus audit covers 22 complete non-English or bilingual corpora: **all 1,348 of 1,348 nodes contain stored English legal/reference text**. **1,322 are aligned to the current-effective source layer**; the remaining **26** are Taiwan PDPA English references aligned to the latest-promulgated amendments whose commencement date has not yet been set, and are explicitly marked as not current-effective. Korea's AI Framework Act is 49/49 aligned to the phase effective 21 July 2026. APPI is 208/208 current-aligned: 172 units use a government reference whose Japanese source text was verified as unchanged, and the 15 changed main-Article/table units plus 21 supplementary blocks use separately labelled project references. Government, public-sector and project-authored references remain distinct; concept mappings and summaries are never substituted as English legal text.

“Complete corpus” means complete within the documented version, language, lifecycle and redistribution boundary. It does not imply that an instrument is currently in force or that every official source may be mirrored. Recitals, annexes, schedules, signatures, amendment instruments or appendices are included only where the relevant manifest expressly says so. Nineteen framework, guidance, proposal or historical records remain selected-source indexes because of reuse restrictions, scope boundaries, or the absence of a verified redistributable edition. `source-audit.json` records the boundary separately for every instrument.
...
The Research Lab defaults to the **39 version-locked complete corpora** and to **current-effective provisions classified as substantive** for the project's subject matter. Rule-generated concept links remain machine candidates and retain that provenance in the analytical data; they are not presented as human-reviewed equivalence findings. Controls can reveal selected-source instruments and structural or other temporal layers, but incomplete coverage is never silently mixed into the default comparative sample.
