...
The corpus reference date is **2026-07-28**. Individual instruments, translations, relations and source audits retain their own review dates; the reference date does not imply a fresh same-day merits review of every source. This is a research aid, not legal advice or a compliance determination.

## Snapshot scope

The current seed contains **30 jurisdiction or issuing-context records, 58 instruments, 255 curated source-linked anchors, 3,134 merged provision/framework nodes, 76 qualified relations, 120 lifecycle events, 58 source-audit records, 23 controlled core concepts in 7 themes, and 34 curated structure summaries**. The merged count overlays 39 version-locked complete corpus datasets with `provisions.json` and de-duplicates stable IDs; it is not the raw length of any one file.

Thirty-nine instruments have a complete, version-locked local corpus within their documented language, lifecycle and redistribution boundary. Coverage includes the principal indexed statutes and regulations, selected historical enactments and bills, and NIST AI RMF 1.0. The remaining nineteen records are framework, guidance, proposal or historical indexes with selected source text or source-linked editorial anchors. “Complete” is version-specific: it does not turn a vetoed bill into law, a voluntary framework into a duty, or a non-authoritative translation into controlling text.
...
- `provision-concepts.json`: one provenance-labelled topic classification per merged node, including highlight state, reviewed and candidate Core Concept IDs, rationale, classification basis and date. In this snapshot, 44 records have an explicit editorial-review layer and 3,090 are machine candidates; these labels do not establish legal equivalence.
...
- `eu-ai-act-annexes.json`: all 14 current Annexes after applying Regulation (EU) 2026/1744 operation by operation.
- `eu-ai-act-recitals.json`: the 180 non-operative explanatory Recitals of Regulation (EU) 2024/1689; Recitals of the 2026 amending act remain attached to that separate act.

The generated EU corpus files are intentionally separate from this hand-curated seed. An application should merge all provision sources by stable `id`, prefer the generated source-derived legal-text record for text, preserve curated relation/status metadata from `provisions.json`, and keep operative Articles, operative Annexes and non-operative Recitals analytically distinct.
...
The current coverage distribution is 39 complete corpora, one selected-source-text-and-index record, and 18 selected-provision indexes based on editorial summaries and official links. Coverage modes distinguish complete verified imports from metadata and selected-anchor review; none represents expert legal advice or silently upgrades an English summary into a translation. A complete corpus may still be historical, vetoed, pending, voluntary, non-authoritative in translation, or subject to phased commencement.
...
1. **EU AI Act amendment boundary:** Regulation (EU) 2026/1744 was published on 24 July 2026 and entered into force on 27 July 2026. The local 119-Article and 14-Annex layers are a source-derived project consolidation assembled operation by operation from authentic Official Journal texts; they are not represented as an independently published EUR-Lex consolidation. Article-level applicability preserves the amended schedule, including 2 August 2026, 2 December 2026, 2 December 2027 and 2 August 2028 layers. The 180 stored Recitals remain the non-operative explanatory Recitals of the 2024 enactment; the amending act's Recitals are not inserted into that sequence.
...
8. **South Korea:** The AI Framework Act has been in force since 22 January 2026 and is not a draft. The locally displayed current corpus now incorporates the amendment phase effective 21 July 2026 (46 Articles plus three supplementary-provision blocks). PIPA separately retains future 2026 and 2027 amendment phases in its version manifest.
